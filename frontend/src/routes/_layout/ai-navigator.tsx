...
import { Bot, Compass, Loader2, MoveRight, Sparkles } from "lucide-react"
...
export const Route = createFileRoute("/_layout/ai-navigator")({
  component: AiNavigatorPage,
  head: () => ({
    meta: [
      {
        title: "AI 導覽 - Campus Cloud",
      },
    ],
  }),
})
...
function AiNavigatorPage() {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-border/70 bg-background/80 p-6">
      ...
      {result ? (
        <section className="space-y-4 rounded-2xl border border-border/70 bg-background/80 p-6">
          ...
          {result.primary ? (
            <div className="rounded-xl border border-primary/35 bg-primary/5 p-4">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="space-y-1">
                  <p className="text-sm text-muted-foreground">主要建議</p>
                ...
                <Button onClick={() => void goToPath(result.primary!.path)}>
                  前往頁面
          ...
          {result.suggestions.length > 0 ? (
            <div className="space-y-3">
              <p className="text-sm font-medium">候選頁面</p>
              <div className="grid gap-3 md:grid-cols-2">
                {result.suggestions.map((item) => (
                  <button
                    ...
                    className="text-left rounded-xl border border-border/70 bg-background px-4 py-3 transition-colors hover:border-primary/35 hover:bg-primary/5"
                  >
